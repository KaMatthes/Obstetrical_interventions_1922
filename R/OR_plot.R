...
Mod_laus_ep <- glm(Episiotomy ~  sex + parity + Position_normal + age_mother + height_weight_quan+ head_Bassin_ConjExt_quan 
                   + GA_weeks + height_cretes_quan ,
                   data = data_laus, family="binomial")


Mod_basel_ep <- glm(Episiotomy ~  sex + parity + Position_normal + age_mother + height_weight_quan+ head_Bassin_ConjExt_quan
                    + GA_weeks + height_cretes_quan,
...
write.xlsx(table_data_laus_ep ,paste0("output/table_data_laus_ep.xlsx"), rowNames=FALSE, overwrite = TRUE)
...
write.xlsx(table_data_basel_ep ,paste0("output/table_data_basel_ep.xlsx"), rowNames=FALSE, overwrite = TRUE)
...
Mod_basel_me <- glm(Mecanisme_normal  ~  sex + parity + Position_normal + age_mother +height_weight_quan+ head_Bassin_ConjExt_quan+ 
                      + GA_weeks + height_cretes_quan,
                    data = data_basel, family="binomial")
...
cowplot::save_plot("output/plot_mec.pdf", plot_mec,base_height=7,base_width=14)
...
Mod_laus_or <- glm(Duree_2me_periode_z  ~  sex + parity + Position_normal + age_mother + head_Bassin_ConjExt_quan +
                     height_weight_quan  + GA_weeks + height_cretes_quan ,
                   data = data_laus)
...
write.xlsx(table_data_laus_du ,paste0("output/table_data_laus_du.xlsx"), rowNames=FALSE, overwrite = TRUE)
...
write.xlsx(table_data_basel_du ,paste0("output/table_data_basel_du.xlsx"), rowNames=FALSE, overwrite = TRUE)
