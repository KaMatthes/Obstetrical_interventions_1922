or_plot_2 <- function (.data, dependent, explanatory, 
          ...
          table_opts = NULL, ...) 
{
  ...
  df.out <- df.out %>%
    ...
    mutate(levels = recode(levels, "[1.0,18.0]" ="-",
                           "[22.0,40.0]" ="-",
                           "[10.9,39.2]" ="-"))
  ...
  t1 = ggplot(df.out, aes(x = as.numeric(OR), y = fit_id)) + 
    annotate("text", x = column_space[1], y = df.out$fit_id, 
             label = df.out[, 2], hjust = 0, size = c(rep(4,dim(df.out)[1] -1), 5)) + 
    annotate("text", x = column_space[2], y = df.out$fit_id,
             label = df.out[, 3], hjust = 1, size = c(rep(4,dim(df.out)[1] -1), 5)) + 
    annotate("text", x = column_space[3], y = df.out$fit_id,
             label = df.out[, 6], hjust = 1, size = c(rep(4,dim(df.out)[1] -1), 5)) + 
    annotate("text", x = column_space[4], y = df.out2$fit_id,
             label = df.out2[, 6], hjust = 1, size = c(rep(4,dim(df.out)[1] -1), 5)) + 
    theme_classic(11) + 
