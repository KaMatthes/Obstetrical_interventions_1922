...
explanatory = c( "sex","parity","Position_normal","Bassin_Cretes","birthweight100",
...
Mod_laus_ep <- glm(Episiotomy ~  sex + parity + Position_normal +  Bassin_Cretes+birthweight100+ 
                     GA_weeks +  head_ConjExt +      dura_terc,
                   data = data_laus, family="binomial")

Mod_basel_ep <- glm(Episiotomy ~  sex + parity + Position_normal +  Bassin_Cretes+birthweight100+ 
                    GA_weeks +   head_ConjExt +      dura_terc,
...
cowplot::save_plot("output/plot_cretes_epi.pdf", plot_epi,base_height=5,base_width=14)
...
write.xlsx(table_data_laus_ep ,paste0("output/table_data_laus_cretes_ep.xlsx"), rowNames=FALSE, overwrite = TRUE)
...
Mod_laus_or <- glm(Duree_2me_periode_z  ~  sex + parity + Position_normal +  Bassin_Cretes+birthweight100+ 
                     GA_weeks +   head_ConjExt,
                   data = data_laus)


Mod_basel_or <- glm(Duree_2me_periode_z  ~   sex + parity + Position_normal +  Bassin_Cretes+birthweight100+ 
                      GA_weeks +   head_ConjExt,
                    data = data_basel)
...
write.xlsx(table_data_laus_du ,paste0("output/table_data_laus_cretes_du.xlsx"), rowNames=FALSE, overwrite = TRUE)
...
write.xlsx(table_data_basel_du ,paste0("output/table_data_basel_cretes_du.xlsx"), rowNames=FALSE, overwrite = TRUE)
