...
Mod_laus_ep <- data.frame(summary(glm(Episiotomy ~  head_circ + birthweight +Bassin_Epines +Bassin_Cretes +Bassin_ConjExt, data = data_laus, family="binomial"))$coefficients) %>%
...
Mod_basel_ep <- data.frame(summary(glm(Episiotomy ~  head_circ + birthweight + Bassin_Epines +Bassin_Cretes+Bassin_ConjExt, data = data_basel, family="binomial"))$coefficients) %>%
  mutate(Est = round(exp(Estimate),2),
         CIl = round(exp(Estimate - 1.96*`Std..Error`),2),
         CIu = round(exp(Estimate + 1.96*`Std..Error`),2),
         Fac = row.names(.)) %>%
...
Mod_laus_me <- data.frame(summary(glm(Mecanisme_normal~  head_circ + birthweight+ Bassin_Epines +Bassin_Cretes+Bassin_ConjExt, data = data_laus, family="binomial"))$coefficients) %>%
  mutate(Est = round(exp(Estimate),2),
         CIl = round(exp(Estimate - 1.96*`Std..Error`),2),
         CIu = round(exp(Estimate + 1.96*`Std..Error`),2),
         Fac = row.names(.)) %>%
...
Mod_basel_me <- data.frame(summary(glm(Mecanisme_normal ~  head_circ + birthweight+ Bassin_Epines +Bassin_Cretes+Bassin_ConjExt, data = data_basel, family="binomial"))$coefficients) %>%
  mutate(Est = round(exp(Estimate),2),
         CIl = round(exp(Estimate - 1.96*`Std..Error`),2),
         CIu = round(exp(Estimate + 1.96*`Std..Error`),2),
         Fac = row.names(.)) %>%
...
Mod_laus_po <- data.frame(summary(glm(Position_normal~  head_circ + birthweight +Bassin_Epines +Bassin_Cretes+Bassin_ConjExt, data = data_laus, family="binomial"))$coefficients) %>%
  mutate(Est = round(exp(Estimate),2),
         CIl = round(exp(Estimate - 1.96*`Std..Error`),2),
         CIu = round(exp(Estimate + 1.96*`Std..Error`),2),
         Fac = row.names(.)) %>%
...
Mod_basel_po <- data.frame(summary(glm(Position_normal ~  head_circ + birthweight + Bassin_Epines +Bassin_Cretes+Bassin_ConjExt, data = data_basel, family="binomial"))$coefficients) %>%
  mutate(Est = round(exp(Estimate),2),
         CIl = round(exp(Estimate - 1.96*`Std..Error`),2),
         CIu = round(exp(Estimate + 1.96*`Std..Error`),2),
         Fac = row.names(.)) %>%
...
cowplot::save_plot("output/OR_plot.pdf", OR_plot,base_height=8,base_width=20)  
